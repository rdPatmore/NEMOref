...
CONTAINS
   ...
   SUBROUTINE usrdef_sbc_oce( kt )
      ...
      REAL(wp) ::   maxlat                ! maximum latitude in km from centre
!      REAL(wp) ::   dy                    ! grid spacing in y
      !!---------------------------------------------------------------------
      zyydd = REAL(nyear_len(1),wp)

!      NAMELIST/namusr_def/ dy
...
!      DO jj = 1, jpj
!         DO ji = 1, jpi
!           !utau(ji,jj) = ztau * COS( rpi/2 * gphiv(ji,jj) / maxlat )
!           utau(ji,jj) = 0.0
!           vtau(ji,jj) = 0.0
!         END DO
!      END DO
...
!      END DO
      ...
      !CALL lbc_lnk_multi( 'usrdef_sbc', utau(:,:), 'T', 1. , vtau(:,:), 'T', 1. )
